...
SELECT 
    Ship_Mode, 
    SUM(TRY_CAST(Shipping_Cost AS FLOAT)) AS [Total Shipping_Cost]
FROM 
    [dbo].[KMS Sql Case Study]
GROUP BY 
    Ship_Mode
ORDER BY 
    [Total Shipping_Cost] DESC;
...
SELECT 
    ...
    AVG(CAST(Shipping_Cost AS FLOAT)) AS Avg_Shipping_Cost
FROM [dbo].[KMS Sql Case Study]
...
For instance, critical orders which are urgent requires fastest delivery whereas, they mostly use slow, yet expensive Delivery Truck.
